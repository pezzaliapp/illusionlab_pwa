// IllusionLab v1.1.3 — i18n, timer, leaderboard, explanations + fixes + improved Duck/Rabbit
(function(){
  ...
  if (modal) modal.hidden = true; // startup fix
  ...
  const illusions = [
    // 1 Blivet
    {
      ...
      correct:2,
      explanation:{it:"Le connessioni tra punte e barra sono ambigue: alcune linee suggeriscono tre cilindri, altre due barre piatte. È un oggetto impossibile.",
      ...
      render:(node)=>{
        node.innerHTML = `
        <svg viewBox="0 0 400 220" width="360" role="img"><g stroke="#eaf1ff" fill="none" stroke-width="6" stroke-linecap="square">
    ...
    // 2 Necker
    {
      ...
      correct:2,
      explanation:{it:"Figura bistabile: l'interpretazione della profondità si inverte spontaneamente.",
    ...
    // 3 Penrose triangle
    {
      ...
      correct:1,
      explanation:{it:"Le giunzioni violano la geometria euclidea: può essere costruito solo come trucco prospettico.",
    ...
    // 4 Penrose stairs
    {
      ...
      correct:2,
      explanation:{it:"Ciclo chiuso che collega piani incongruenti: l'altitudine non cambia mai.",
    ...
    // 5 Ames
    {
      ...
      correct:1,
      explanation:{it:"La stanza è deformata: il pavimento e le pareti non sono ortogonali all'osservatore.",
    ...
    // 6 Rubin
    {
      id:"rubin", title:{it:"Vaso di Rubin", en:"Rubin Vase"},
      ...
      correct:2,
      explanation:{it:"Ambiguità figura/sfondo: non puoi percepire simultaneamente vaso e profili come figure.",
    ...
    // 7 Duck/Rabbit — improved
    {
      ...
      correct:2,
      explanation:{it:"La stessa sagoma supporta due letture: a destra un becco d’anatra, che ruotato mentalmente diventa orecchie da coniglio.",
                   en:"The same outline supports two readings: a duck beak to the right that, when mentally rotated, becomes rabbit ears."},
      render:(node)=>{
        node.innerHTML = `
        <svg viewBox="0 0 320 180" width="300" role="img" aria-label="Duck or Rabbit ambiguous figure">
          <!-- head -->
          <ellipse cx="150" cy="95" rx="70" ry="45" fill="none" stroke="#eaf1ff" stroke-width="6"/>
          <!-- eye -->
          <circle cx="130" cy="88" r="5" fill="#eaf1ff"/>
          <!-- beak/ears shape (ambiguous) -->
          <path d="M200 85 Q255 65 285 85 Q255 105 200 100 Z" fill="none" stroke="#eaf1ff" stroke-width="6" stroke-linejoin="round"/>
          <!-- small tail/whisker cue to the left to help flip -->
          <path d="M95 95 Q75 105 60 98" fill="none" stroke="#eaf1ff" stroke-width="6"/>
        </svg>
        <div class="helper">${lang==="it"
          ?"Ruota mentalmente la figura: il becco a destra diventa due orecchie; l’occhio resta valido in entrambe le letture."
          :"Mentally rotate: the right beak becomes two ears; the eye works for both interpretations."}</div>`;
      },
      hint:(node)=>{ node.querySelector(".helper").textContent = lang==="it"?"Copri la coda a sinistra: l’anatra diventa coniglio. Copri il becco a destra: torna anatra.":"Cover the left tail to see a rabbit; cover the right beak to see a duck."; }
    },
    // 8 Kanizsa
    {
      id:"kanizsa", title:{it:"Quadrato di Kanizsa", en:"Kanizsa Square"},
      ...
      correct:1,
      explanation:{it:"Il sistema visivo completa contorni assenti (contorni illusori).",
    ...
    // 9 Müller-Lyer
    {
      ...
      correct:0,
      explanation:{it:"Le frecce alterano il contesto prospettico, ma le linee centrali coincidono.",
      ...
      hint:(node)=>{ node.innerHTML += `<svg viewBox="0 0 360 40" width="340"><g stroke="#22c55e" stroke-width="2"><line x1="60" y1="20" x2="300" y2="20"/></g></svg>`; }
    },
    // 10 Three or Four bars
    {
      id:"bars34", title:{it:"Tre o Quattro?", en:"Three or Four?"},
      ...
      correct:2,
      explanation:{it:"Ambiguità di segmentazione prospettica: la parte frontale non si mappa univocamente con quella posteriore.",
